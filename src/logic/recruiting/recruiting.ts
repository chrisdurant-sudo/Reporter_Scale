...
function actions(snapshot: DemoSnapshotV2, acq: AcquisitionCase, asOf: UtcTimestamp): readonly RecruitingAction[] {
  ...
  for (const work of snapshot.workItems.filter((item) => ms(item.createdAt) <= ms(asOf) && openWork(item, asOf) && ((item.primaryEntityRef.kind === "acquisition-case" && item.primaryEntityRef.id === acq.id) || (item.primaryEntityRef.kind === "reporter" && item.primaryEntityRef.id === acq.reporterId)))) { const owner = work.ownerHistory.filter((change) => ms(change.occurredAt) <= ms(asOf)).sort((left, right) => ms(left.occurredAt) - ms(right.occurredAt)).at(-1)?.ownerId ?? null; const overdue = work.dueAt !== null && ms(work.dueAt) < ms(asOf); result.push({ kind: "work-item", label: work.kind.replaceAll("-", " "), reason: work.blockerCode ? `Blocked: ${work.blockerCode}` : overdue ? "Open work is overdue" : owner === null ? "Needed action is unowned" : "Concrete work remains open", waitingOnKey: work.blockerCode ? `blocker:${work.blockerCode}` : `work:${work.kind}`, waitingOnLabel: work.blockerCode ? `Blocked: ${work.blockerCode}` : work.kind.replaceAll("-", " "), assignedTo: owner, dueAt: work.dueAt, record: { kind: "work-item", id: work.id } }); }
  return result;
}
...
export function funnelWaitTimeTrends(snapshot: DemoSnapshotV2, asOfAt: UtcTimestamp, selectedMarket: "ALL" | MarketId = "ALL"): FunnelWaitTimeTrends {
  ...
  const series = (range: "R7" | "R28") => Object.fromEntries(FUNNEL_SLA_STATUSES.map((status) => [status, trendOffsets(range).map((offset) => {
