...
function fixture(): DemoSnapshotV2 {
  const people = Array.from({ length: 40 }, (_, index) => {
    ...
    return {
      reporter: { id, fictionalName: `Fictional ${index + 1}`, recruitingMarketId: market, serviceMarketIds: [market], createdAt: "2025-12-01T00:00:00.000Z", recordedAt: "2025-12-01T00:00:00.000Z", preferences: { attendanceModes: ["remote"], supportedProceedingTypes: [], supportedCapabilityCodes: [], serviceMarkets: [], notes: "" }, provenance: "synthetic-demo" },
      ...
      assignment: { id: assignmentId, requestId: `request-${index + 1}` as never, reporterId: id, state: "accepted", occurredAt: "2026-01-09T00:00:00.000Z", recordedAt: "2026-01-09T00:00:00.000Z", actorId: "actor-1" as never, source: "synthetic-seed", reason: "fixture", provenance: "synthetic-demo" },
      job: successful ? { id: `job-${index + 1}` as never, requestId: `request-${index + 1}` as never, reporterId: id, acceptedAssignmentEventId: assignmentId, outcome: "completed", startedAt: "2026-01-09T12:00:00.000Z", completedAt: "2026-01-10T00:00:00.000Z", deliveryAt: null, recordedAt: stamp, provenance: "synthetic-demo" } : null,
    };
  });
  return {
    schemaVersion: 2, seedVersion: "test", revision: 3, baseAsOfAt: stamp as never, currentAsOfAt: stamp as never, appliedCommandIds: [], appliedScenarioEventIds: [], markets: [], metricDefinitions: [{ id: "M12" as never, version: "v2-frozen-1" as never, unit: "ratio", description: "Checklist", timeWindowSemantics: "14 days", populationRule: "Explicit", attributionRule: "Global first job" }, { id: "M13" as never, version: "v2-frozen-1" as never, unit: "events", description: "Weekly and goal review integrity", timeWindowSemantics: "Explicit weekly window", populationRule: "Canonical review records", attributionRule: "Frozen review scope" }], evidenceSnapshots: [{ id: "evidence-1", denominatorMembers: people.filter((item) => item.enrollment.groupId === "pilot").map((item) => ({kind: "program-enrollment", id: item.enrollment.id})) }], manualMarketNotes: [],
    reporters: people.map((item) => item.reporter), acquisitionCases: people.map((item) => item.case), lifecycleEvents: people.map((item) => item.lifecycle), credentialRecords: [], capabilityVerifications: [], screeningReviews: [], onboardingSteps: [], readinessEvents: [], availabilityWindows: [], demandRequests: people.map((item) => ({ id: item.assignment.requestId, marketId: item.enrollment.marketAtEntry, createdAt: "2026-01-08T00:00:00.000Z", recordedAt: "2026-01-08T00:00:00.000Z", startAt: "2026-01-09T12:00:00.000Z", endAt: "2026-01-09T13:00:00.000Z" })), assignmentEvents: people.map((item) => item.assignment), jobOutcomes: people.flatMap((item) => item.job ? [item.job] : []), teamMembers: [], workItems: [], teamTargets: [], workQualityChecks: [], coachingActions: [],
    ...
    programs: [{ id: programId, title: "Readiness checklist", marketIds: ["LAX", "SFO"], linkedNeedRefs: [], type: "workflow", stage: "reviewing", ownerId: "team-1" as never, hypothesis: "Synthetic hypothesis", changeSummary: "Checklist", primaryMetric: { id: "M12" as never, version: "v2-frozen-1" as never }, targetRef: "goal-1" as never, startAt: stamp as never, reviewAt: stamp as never, measurementPlan: { metric: { id: "M12" as never, version: "v2-frozen-1" as never }, entryWindow: { startAt: "2026-01-01T00:00:00.000Z" as never, endAt: "2026-01-02T00:00:00.000Z" as never, boundary: "[start,end)" }, followUpDays: 14, eligibilityRule: "Complete histories", attributionRule: "Source fixed at entry" }, originWorkaroundRef: null, limitations: ["Non-random synthetic sample"], provenance: "synthetic-demo" }],
    programEnrollments: people.map((item) => item.enrollment), programNotes: [], programDecisions: [], goalRevisions: [{ id: "goal-revision-1" as never, goalId: "goal-1" as never, version: 1, metric: { id: "M12" as never, version: "v2-frozen-1" as never }, scope: { marketIds: ["LAX", "SFO"], programIds: [programId], acquisitionCasePurpose: "first-time", requiredCapabilityCodes: [] }, baselineAsOfAt: stamp as never, baselineEvidenceSnapshotId: "evidence-1" as never, target: 0.5, deadline: stamp as never, ownerId: "team-1" as never, savedAt: stamp as never, changeReason: "Fixture", supersedesRevisionId: null, provenance: "synthetic-demo" }], workaroundExamples: [], processVersions: [], commandRecords: [],
...
describe("prepareProgramsView", () => {
  ...
  it("prepares market-scoped locked rows, summaries, notes, workflow sources, and ordered cohort trend points", () => {
    ...
    expect(lax.resultOverTime[0]).toMatchObject({ target: null, targetState: "not-declared", evidenceId: expect.stringContaining("evidence-program") });
    expect(lax.resultOverTime[1]).toMatchObject({ targetState: "not-declared" });
    expect(lax.resultOverTime[1]!.accessibleLabel).toMatch(/no compatible declared target/i);
  ...
  it("attributes exact source spend once, never treats missing spend as free, and preserves historic goal revisions", () => {
    ...
    expect(projection.revisions.map((revision) => [revision.version, revision.target, revision.metricVersion])).toEqual([[1, 0.5, "v2-frozen-1"], [2, 0.7, "v2-frozen-1"]]);
  ...
  it("prepares a partial weekly review from canonical work and dated decisions with exact evidence", () => {
    ...
    expect(review.evidence.filter((item) => item.id.includes("evidence-weekly-")).every((item) => item.metric.id === "M13" && item.metric.version === "v2-frozen-1")).toBe(true);
