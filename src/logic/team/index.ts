...
export interface TeamMemberView {
  ...
  readonly cycleTime: {
    readonly completedSamples: readonly { readonly workItemId: string; readonly label: string; readonly createdAt: string; readonly completedAt: string; readonly elapsedHours: number }[];
    readonly waitingSamples: readonly { readonly workItemId: string; readonly label: string; readonly createdAt: string; readonly ageHours: number }[];
  };
...
export function prepareTeamView(snapshot: DemoSnapshotV2, context: WorkspaceQueryContext<typeof TEAM_WORKSPACE>, metric: MetricDefinitionRef): PreparedTeamView {
  ...
  const members = snapshot.teamMembers.filter((member) => beforeOrAt(member.activeFrom, asOf) && (member.activeTo === null || stamp(member.activeTo) > stamp(asOf))).map((member) => {
    ...
    evidence.push(countEvidence(`team-waiting-${member.id}`, metric, context, assigned, "Still-waiting ages are current open work owned at the selected as-of time."));
