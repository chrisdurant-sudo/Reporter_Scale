...
export function validateDemoSnapshot(value: unknown): RepositoryResult<DemoSnapshotV2> {
  ...
  if (candidate.assignmentEvents.some((item) => {
    ...
    const readiness = candidate.readinessEvents.filter((event) => event.reporterId === item.reporterId && !after(event.occurredAt, item.occurredAt) && !after(event.recordedAt, item.occurredAt)).sort((left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt))[0];
    ...
    if (!reporter.serviceMarketIds.includes(request.marketId) || !reporter.preferences.supportedProceedingTypes.includes(request.proceedingType) || !reporter.preferences.attendanceModes.includes(request.attendanceMode) || request.status === "canceled" || request.sampleCredentialRequirements.some((requirement) => !requirement.requirementCode || !requirement.label)) { invalidAssignmentId = String(item.id); return true; }
    const latestLifecycle = candidate.lifecycleEvents.filter((event) => event.reporterId === item.reporterId && !after(event.occurredAt, item.occurredAt) && !after(event.recordedAt, item.occurredAt)).sort((left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt))[0];
    if (latestLifecycle?.eventType === "closed") { invalidAssignmentId = String(item.id); return true; }
    if ([...requiredCapabilities].some((code) => {
      const latest = candidate.capabilityVerifications.filter((capability) => capability.reporterId === item.reporterId && capability.capabilityCode === code && !after(capability.recordedAt, item.occurredAt)).sort((left, right) => Date.parse(right.recordedAt) - Date.parse(left.recordedAt))[0];
