...
for (const [marketIndex, market] of (["LAX", "SFO", "DFW", "ORD", "ATL"] as const).entries()) {
  ...
  for (const row of rows) {
    ...
    const screeningAt = row.offset === 1 || row.stage === "screening-bottleneck" ? currentStageAt : p4DateAt(timelineStart, 3);
    const onboardingAt = row.offset === 3 || row.stage === "onboarding-bottleneck" ? currentStageAt : p4DateAt(timelineStart, 5);
    ...
    if (row.stage === "screening" || row.stage === "screening-bottleneck" || hasOnboarding) {
      ...
      screeningReviews.push({ id: asId(`screening-p4-${market.toLowerCase()}-${suffix}`), reporterId: asId(reporterId), acquisitionCaseId: asId(caseId), checks: [{ checkCode: "p4-capability", required: true, status: pending ? "needs-information" : "complete", evidenceRef: pending ? null : { kind: "reporter", id: reporterId }, note: pending ? "Fictional capability evidence remains unrecorded." : "Fictional screening evidence recorded." }], outcome: pending ? "needs-information" : "verified", unresolvedInformation: pending ? ["p4-capability"] : [], reviewerId: asId("team-1"), reviewedAt: utc(screeningAt), recordedAt: utc(screeningAt), reason: pending ? "Awaiting defined fictional evidence." : "Fictional screening review completed.", provenance });
    }
    if (hasOnboarding) {
      const blocked = row.stage === "onboarding-bottleneck";
      onboardingSteps.push({ id: asId(`step-p4-${market.toLowerCase()}-${suffix}`), acquisitionCaseId: asId(caseId), stepDefinitionId: "p4-requirements", required: true, state: blocked ? "blocked" : "completed", assignedTo: asId("team-1"), dueAt: utc(isReady ? readyAt : p4DateAt(onboardingAt, 1)), completedAt: blocked ? null : utc(isReady ? readyAt : p4DateAt(onboardingAt, 1)), completedBy: blocked ? null : asId("team-1"), evidenceRef: blocked ? null : { kind: "reporter", id: reporterId }, blockerCode: blocked ? "p4-required-evidence-missing" : null, recordedAt: utc(isReady ? readyAt : onboardingAt), provenance });
    }
    if (isReady) {
