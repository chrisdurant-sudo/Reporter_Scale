...
for (let number = 1; number <= 10; number += 1) {
  ...
  if (number <= 8) {
    ...
    assignmentEvents.push({ id: asId(historyAssignmentId), requestId: asId(historyRequestId), reporterId: asId(reporterId), state: "accepted", occurredAt: utc("2025-12-31T17:00:00Z"), recordedAt: utc("2025-12-31T17:00:00Z"), actorId: asId("actor-team-3"), source: "synthetic-seed", reason: "Fictional prior accepted work establishes an existing reporter history.", provenance });
...
export function validateDemoSnapshot(value: unknown): RepositoryResult<DemoSnapshotV2> {
  ...
  const capabilityIds = new Set(candidate.capabilityVerifications.map((item) => item.id));
  const stepIds = new Set(candidate.onboardingSteps.map((item) => item.id));
  ...
  if (candidate.readinessEvents.some((item) => caseById.get(item.acquisitionCaseId)?.reporterId !== item.reporterId || item.capabilityVerificationIds.some((idValue) => !capabilityIds.has(idValue)) || item.checkedStepIds.some((idValue) => !stepIds.has(idValue)) || after(item.occurredAt, item.recordedAt) || after(item.recordedAt, candidate.currentAsOfAt))) return failure(candidate, "validation-failed", "Readiness prerequisites or timestamps are invalid.", "readinessEvents");
  ...
  if (candidate.demandRequests.some((item) => !marketIds.has(item.marketId) || !after(item.endAt, item.startAt) || after(item.recordedAt, candidate.currentAsOfAt))) return failure(candidate, "validation-failed", "Demand references or interval are invalid.", "demandRequests");
  if (candidate.assignmentEvents.some((item) => !reporterIds.has(item.reporterId) || !requestById.has(item.requestId) || after(item.occurredAt, item.recordedAt) || after(item.recordedAt, candidate.currentAsOfAt))) return failure(candidate, "validation-failed", "Assignment references or timestamps are invalid.", "assignmentEvents");
  if (candidate.jobOutcomes.some((item) => { const request = requestById.get(item.requestId); const assignment = assignmentById.get(item.acceptedAssignmentEventId); return !request || !assignment || assignment.state !== "accepted" || assignment.requestId !== item.requestId || assignment.reporterId !== item.reporterId || (item.outcome === "completed" && (item.completedAt === null || after(request.endAt, item.completedAt))) || after(item.recordedAt, candidate.currentAsOfAt); })) return failure(candidate, "validation-failed", "Job outcome references, acceptance, or chronology are invalid.", "jobOutcomes");
