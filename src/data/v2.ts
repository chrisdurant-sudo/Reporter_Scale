...
export function validateDemoSnapshot(value: unknown): RepositoryResult<DemoSnapshotV2> {
  ...
  if (candidate.readinessEvents.some((item) => {
    ...
    if (checkedSteps.some((step) => !step || step.acquisitionCaseId !== item.acquisitionCaseId || !step.required || step.state !== "completed" || step.completedAt === null || step.evidenceRef === null || after(step.completedAt, item.occurredAt) || after(step.recordedAt, item.occurredAt))) return true;
  ...
  if (candidate.assignmentEvents.some((item) => {
    ...
    const historicalCompletion = candidate.jobOutcomes.some((outcome) => outcome.acceptedAssignmentEventId === item.id && outcome.outcome === "completed" && outcome.completedAt !== null && after(outcome.completedAt, item.occurredAt));
    if (historicalCompletion) return false;
    const readiness = candidate.readinessEvents.filter((event) => event.reporterId === item.reporterId && !after(event.occurredAt, item.occurredAt)).sort((left, right) => Date.parse(right.occurredAt) - Date.parse(left.occurredAt))[0];
