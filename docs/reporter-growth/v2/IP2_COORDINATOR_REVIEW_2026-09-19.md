...
## Retained browser regression discovered by Quality

The aligned exact-source browser suite reaches the preserved XR23 narrow evidence-drawer check and reports width370.5px at390×844, below the existing389px minimum. Height844, Escape, focus return and page overflow pass. Seven of eight browser cases pass. The source is unchanged from12690e; the shared `.ip2-drawer`95vw maximum lacks the former fullscreen narrow override. Raw diagnostic: `evidence/12690e87b4a43366c6c6f04509823ad13d1973d0-drawer-regression.json`.

This is an actual regression, not an obsolete assertion. Keep the assertion. After Quality stops, authorize only the minimal shared CSS correction in `src/styles/tokens.css` to retain fullscreen narrow drawers without changing desktop layout. This restores an existing browser requirement under the desktop-scope document; it does not open a mobile optimization or dedicated capture pass. Full suite and clean desktop proof follow the corrected source.

## Final integrated IP2 verification

Exact clean source `055fcd18a39a6238d6beb7fc52bdd03a315ea1c3` fixes the retained390px drawer-width assertion with one shared CSS rule; Quality tests remain unchanged. The coordinator accepted the one-path lane audit and integrated at `b5cbe33e66a52f466456467a240d2f704208fb10`. Full `npm run verify:p4` passes:203 governance checks, lint, types,315 unit/integration,9 acceptance, build,8 browser. No waivers.

Fresh desktop proof contains20 checksummed screenshots and22 passing interaction checks.18 images are byte-identical to the prior reviewed package; the two changed-byte SLA/person drawer captures were directly compared and preserve the reviewed desktop geometry and content. Root and Lead review found no remaining visual defect. The final versioned package is `docs/reporter-growth/v2/design-lock/candidate/055fcd18a39a6238d6beb7fc52bdd03a315ea1c3/proof-manifest.json`. Final phase acceptance follows specialist runtime verification.
