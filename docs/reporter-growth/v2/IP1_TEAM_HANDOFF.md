...
Additional top-level ports are `onEditWork(WorkEditPayload)` and `onTransitionWork(WorkTransitionPayload)`, both awaited canonical transactions. `commandContext: DemoActionContext` supplies the current snapshot revision, command time, active synthetic manager identity and busy state. This is distinct from a historical evidence evaluation. Use its source actor/member for authored records, with a visible acting-as label; disable mutation if absent. `onCreateRecordId(DemoRecordKind)` supplies identities for new target, quality-check and coaching records. Forms never invent actor IDs, clock values or record IDs. Existing payload callbacks remain under `actions`; await/catch every write and retain entered fields on failure.
