...
export const DESKTOP_ONLY_INTERVIEW_SCOPE = 'desktop_only_interview_2026_09_19';
export const interviewTouchRequired = (state) => state.interview_improvement_amendment?.presentation_scope?.id !== DESKTOP_ONLY_INTERVIEW_SCOPE;
export function interviewViewports(state) {
  if (!interviewTouchRequired(state)) return [[1440, 900]];
...
export function inspectInterviewGates(state, registry, verifyEvidence = () => false) {
  ...
  if (presentationScope) {
    check([DESKTOP_INTERVIEW_SCOPE, DESKTOP_ONLY_INTERVIEW_SCOPE].includes(presentationScope.id) && registry.interview_improvement_amendment?.presentation_scope === presentationScope.id,
  ...
  if (index >= 3) {
    ...
    if (interviewTouchRequired(state)) check(proof?.pointer_keyboard_touch_passed === true, 'Visual acceptance requires pointer, keyboard and touch proof.');
    else check(proof?.pointer_keyboard_passed === true, 'Desktop visual acceptance requires pointer and keyboard proof.');
    check(proof?.waivers?.length === 0, 'Visual acceptance must pass without waivers.');
